package com.jitda.global.config.s3.service;
...
public class S3Service {
    ...
    public String upload(MultipartFile multipartFile, String dirName) throws IOException {
        ...
        File uploadFile = convert(multipartFile)
                .orElseThrow(() -> new BadRequestException(ExceptionCode.FILE_TRANSACTION_FAIL));
    ...
    private String upload(File uploadFile, String dirName){
        ...
        String fileName = setFileName(dirName, uploadFile.getName());
    ...
    private String setFileName(String dirName, String originalFileName) {
        String extension = originalFileName.substring(originalFileName.lastIndexOf("."));
        String fileName = dirName + "/" + UUID.randomUUID() + extension;

        if (amazonS3Client.doesObjectExist(bucket, fileName)) {
            fileName = dirName + "/" + UUID.randomUUID() + extension;
        }
    ...
    public File downloadChatFile(String keyName) {
        try {
            ...
            throw new BadRequestException(ExceptionCode.FILE_DOWNLOAD_FAIL);
        }
    }
}
